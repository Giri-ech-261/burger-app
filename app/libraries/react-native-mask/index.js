...
export default class Mask extends React.Component<Props, State> {
  ...
  render() {
    const { children, width, height, shape, borderRadius, wash, borderColor, borderWidth } = this.props;

    // Per default let's assume width and height is given
    const additionalStyles: Array<Object> = [{ width, height, borderColor, borderWidth }];
    ...
    switch (shape) {
      ...
      case 'rounded': {
        ...
        additionalStyles.push({ borderRadius, borderColor, borderWidth });
        break;
      }
      case 'square': {
